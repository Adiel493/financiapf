...
//  DatingApp
//
//  Creado por Adiel Luna on 2/2/19.
...
class ATCProfileViewController: ATCGenericCollectionViewController, ProfileImageTapCollectionViewDelegate {
    ...
    public func update() {
        ...
        allItems.append(ATCProfileButtonItem(title: "Cerrar Sesión".localizedCore,
                                             color: nil,
                                             textColor: uiConfig.mainTextColor))
    ...
    fileprivate func showActionSheet(_ view: UIView) {
        let actionSheet = UIAlertController(title: "Foto de perfil", message: "Actualiza tu foto de perfil", preferredStyle: .actionSheet)
        let removeAction = UIAlertAction(title: "Eliminar foto", style: .destructive) { [weak self] (remove) in
            // Remove photo from the firebase
            guard let self = self else { return }
            self.removePhoto()
        }
        
        let changeAction = UIAlertAction(title: "Cambiar foto", style: .default) { [weak self] (change) in
            guard let self = self else { return }
            self.showChangePhotoOptionsActionSheet()
        }
    
        let cancelAction = UIAlertAction(title: "Cancelar".localizedCore, style: .cancel, handler: nil)
    ...
    fileprivate func showChangePhotoOptionsActionSheet() {
        let actionSheet = UIAlertController(title: "Cambiar foto", message: "Cambiar tu foto de perfil", preferredStyle: UIDevice.current.userInterfaceIdiom == .pad ? .alert : .actionSheet)
        let cameraAction = UIAlertAction(title: "Cámara", style: .default) { [weak self] (camera) in
            //Take photo from camera
            guard let strongSelf = self else { return }
            strongSelf.didTapAddImageButton(sourceType: .camera)
        }
        
        let libraryAction = UIAlertAction(title: "Librería", style: .default) { [weak self] (change) in
    ...
    func removePhoto() {
        ...
        let hud = CPKProgressHUD.progressHUD(style: .loading(text: "Eliminando"))
        hud.show(in: view)
    ...
    func updateProfileImage(image: UIImage) {
        ...
        let hud = CPKProgressHUD.progressHUD(style: .loading(text: "Actualizando"))
        hud.show(in: view)

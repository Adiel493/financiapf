...
class ATCResetPasswordScreenManager: NSObject, ATCResetPasswordScreenDelegate {
    ...
    fileprivate func showResetPasswordSuccess(_ title: String? = "Te acabamos de mandar un email de recuperación de contraseña. Favor de verificar el correo y seguir las instrucciones".localizedCore) {
    ...
    fileprivate func showResetPasswordError(_ title: String? = "El email no es válido. Por favor vuelva a intentar".localizedCore) {
        let alert = UIAlertController(title: title, message: "", preferredStyle: UIAlertController.Style.alert)

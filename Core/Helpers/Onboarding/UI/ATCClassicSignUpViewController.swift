...
class ATCClassicSignUpViewController: UIViewController, ATCSignUpScreenProtocol {
    ...
    override func viewDidLoad() {
        ...
        backButton.addTarget(self, action: #selector(didTapBackButton), for: .touchUpInside)
        let linkedText = "términos y condiciones".localizedCore
        let string = "Al crear una cuenta, estás de acuerdo con nuestros ".localizedCore + linkedText + "."
        let attributedString = NSMutableAttributedString(string: string)
        attributedString.addAttribute(.link,
                                      value: "https://policies.google.com/terms?hl=es",

...
class FinanceStaticDataProvider: NSObject {
    ...
    static let currencyString = "Ingresos: $"
    ...
    static var portfolioCashAccounts: [ATCFinanceAccount] = [
        ...
        ATCFinanceAccount(title: "Mayo 21, 2023",
                          ...
                          amount: "1,721.32",
                          isPositive: true,
                          institution: "Colegiatura Mayo"),
        ATCFinanceAccount(title: "Junio 1, 2023",
                          color: "#342323",
                          logoURL: "https://c93fea60bb98e121740fc38ff31162a8.s3.amazonaws.com/wp-content/uploads/2017/04/robinhood.jpg",
                          amount: "$6,123",
                          isPositive: true,
                          institution: "NU Bank"),
        ATCFinanceAccount(title: "Junio 2, 2023",
                          color: "#342323",
                          logoURL: "https://assets.coingecko.com/markets/images/23/large/fe290a14-ac8f-4c90-9aed-5e72abf271f0.jpeg?1527171545",
                          amount: "$15,712",
                          isPositive: true,
                          institution: "Santander"),
        ATCFinanceAccount(title: "Junio 7, 2023",
                          color: "#342323",
                          logoURL: "https://theme.zdassets.com/theme_assets/586236/49e4904c4910a8ebf63b67d41f9b98b6b0933275.png",
                          amount: "$15,712",
                          isPositive: true,
                          institution: "Coppel"),
        ATCFinanceAccount(title: "Junio 9, 2023",
                          color: "#342323",
                          logoURL: "https://static1.squarespace.com/static/5634e954e4b0f9cec0c16373/t/5bcc7f24085229820cd48c9e/1540128588808/Bofa-logo.jpg",
                          amount: "$10,022",
                          isPositive: true,
                          institution: "HSBC"),
        ATCFinanceAccount(title: "Junio 21, 2023",
                          color: "#342323",
                          logoURL: "https://c93fea60bb98e121740fc38ff31162a8.s3.amazonaws.com/wp-content/uploads/2017/04/robinhood.jpg",
                          amount: "$5,122",
                          isPositive: true,
                          institution: "Colegiatura Junio"),
    ]
...
func getExpenses(completion: @escaping ([ATCExpenseCategory]) -> Void) {
    let collectionRef = Firestore.firestore().collection("expenses")
    
    collectionRef.getDocuments { (querySnapshot, error) in
        if let error = error {
            print("Error al obtener documentos: \(error)")
            completion([]) // Llama a la clausura de finalización con un array vacío en caso de error
        } else {
            var expenses: [ATCExpenseCategory] = []
            ...
            completion(expenses) // Llama a la clausura de finalización con el array de expenses
        }
    }
}

func getIncome(completion: @escaping ([ATCFinanceAccount]) -> Void) {
    let collectionRef = Firestore.firestore().collection("incomes")
    ...
    collectionRef.getDocuments { (querySnapshot, error) in
        if let error = error {
            ...
            for document in querySnapshot!.documents {
                ...
                let isPositive = data["isPositive"] as? String ?? ""
                let fecha = data["date"] as? String ?? ""
                let title = data["title"] as? String ?? ""
                
                let incomeCategory = ATCFinanceAccount(title: fecha, color: color,
                                                         logoURL: logoURL,
                                                         amount: amount,
                                                        isPositive: true,
                                                        institution: title)
                
                incomes.append(incomeCategory)
            }
            
            completion(incomes) // Llama a la clausura de finalización con el array de expenses
        }
    }
}
